...
export default function Transaction({ transaction, handleRemoveTransaction }) {
  ...
  return (
      <li key={transaction.id} className="transaction">
              <div className={`ball ${transaction.category === 'Income' ? 'income-background' : 'expense-background'}`}></div>
                  <div className={`transaction-sum ${transaction.category === 'Income' ? 'income-color' : 'expense-color'}`}>
                      {`₪${transaction.sum}`}
                  </div>
